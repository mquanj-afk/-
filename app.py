...
# --- 🔐 SUPABASEの設定（メモした2つの鍵をここに貼り付けてください） ---
    ...
    if auth_action == "新規会員登録":
        if st.sidebar.button("アカウントを作成する"):
            try:
                ...
                if res.user:
                    st.sidebar.success("登録完了！メールを確認して承認後、ログインしてください。")
            except Exception as e:
...
# ログイン中なら、プロフィール変更時に金庫（データベース）へ自動で保存更新
if st.session_state["user_id"] is not None:
    if st.sidebar.button("💾 このプロフィールで金庫を更新"):
        ...
        try:
            ...
            st.sidebar.error(f"保存失敗（※Supabase側にテーブル作成が必要です）: {e}")
    ...
    if uploaded_file is not None:
        ...
        if st.button("エネルギー・栄養素を計算する"):
            if not api_key:
                ...
                with st.spinner("AIが最先端モデルで秘密のセキュリティ分析中..."):
                    try:
